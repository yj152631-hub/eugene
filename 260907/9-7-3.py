...
st.caption('나이•성별 조건으로 필터링해 보고, 결측치를 제거해 새 CSV로 저장합니다.')
...
missing_age_count = df['Age'].isna().sum() # isna()는 결측치만 True 반환
st.write(f'Age 열의 결측치 개수 : **{missing_age_count}개**')

# Age 열이 결측치인 행만 골라서 제거

df_clean = df.dropna(subset=['Age'])
col1, col2 = st.columns(2)
with col1 :
    st.metric('제거 전', f'{len(df)}명')
with col1 :
    st.metric('제거 후', f'{len(df_clean)}명')
